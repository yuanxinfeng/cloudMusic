<template>
  <div class="dj-fs">
    ...
    <ul>
      <li v-for="(item,index) in privatecontentList" :key="index" :style="{width:fourWidth+'px'}">
...
<script>
import { privatecontent } from "@/api/findMusic";
...
export default {
  ...
  created() {
    privatecontent().then(res => {
      this.privatecontentList.push(...res.result.splice(0, 4));
    });
  },
  methods: {
    clickToDo() {
